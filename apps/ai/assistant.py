...
def system_prompt(site):
    ...
    lines = [
        ...
        "Answer concisely and helpfully about products, shipping, pricing and ordering.",
        ...
        "We hold NO certificate of analysis, NO purity result and NO identity confirmation for any compound. Never claim testing, purity or a COA. If asked, say plainly that we hold none and the material should be treated as uncharacterised.",
...
def stub_answer(question, site):
    ...
    for p in products:
        if p.name.lower() in q or p.slug.replace("-", " ") in q:
            ...
            return (f"{p.name} ({p.category.name}) is ${p.price}/vial in {', '.join(p.sizes)}, "
                    f"currently {stock}. {p.description} "
                    f"We hold no certificate of analysis or purity result for it. {DISCLAIMER}")
    ...
    if any(w in q for w in ["coa", "test", "purity", "quality", "hplc", "lab"]):
        return ("We hold no certificate of analysis, no purity result and no identity "
                "confirmation for any compound in the catalogue, and we don't claim otherwise. "
                "Treat the material as uncharacterised — if your work depends on confirmed "
    ...
    return (f"Happy to help. {brand} carries research compounds across {', '.join(cats)}. "
            f"Ask about a specific compound, pricing or shipping and I'll point you the right way. {DISCLAIMER}")
